    ...
    >
      ...
      <div className="w-full pb-4 grow flex flex-col border-t-[1px] gap-2 dark:bg-black/80">
        <div className="flex justify-between items-center grow min-h-[40px] gap-2 pr-2 bg-white py-2 dark:bg-black/80 dark:text-white ">
          <h2 className="font-titleFont text-base font-bold w-full pl-2 uppercase text-sm h-12" onClick={handleDetails}>
            {title?.substring(0, 15)}
          </h2>
          <div className="text-md relative w-full flex justify-end overflow-hidden self-stretch items-center py-1">
            <div className="flex flex-col-reverse text-xs transform group-hover:translate-x-full group-focus-visible:translate-x-26 transition-transform duration-300">
              {oldPrice ? <p className="line-through text-gray-500">${oldPrice} </p> : null}
              <p className="font-semibold">${price}</p>
            </div>
            <Button
              onClick={() => dispatch(addToCart(product)) & toast.success(`${title} is added in cart`)}
        ...
        <div className="flex flex-col gap-1">
          <p className="uppercase font-semibold text-white bg-blue-900 px-2 w-max rounded-r-lg">{brand}</p>
          <p className="uppercase font-semibold text-white bg-blue-900 px-2 w-max rounded-r-lg">{category}</p>
        </div>
      </div>
